...
who = [
    local list [CN LAG PING IPADDR NAME TIME STATE"\\n"]
    foreach (players) [
        parameters cn
        cty = "unknown"
        try [cty = (countrycode (player_ip $cn))] []
        row = (shell_quote [@cn @(player_lag $cn) @(player_ping $cn) @(player_ip $cn) @(player_name $cn) @(duration (player_contime $cn)) @(player_status $cn)])
