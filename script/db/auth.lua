...
local db, perr, search_for_domain, select_local_domains, is_domain_local, search_for_user, search_for_user_insensitive, select_domain_users, insert_user, insert_domain
...
if using_sqlite then
    ...
    search_for_user_insensitive, perr = db:prepare("SELECT * FROM users WHERE domain_id = :domain_id and lower(name) = :name")
    if not search_for_user_insensitive then error(perr) end
...
function auth.found_name_insensitive(name,domain_id)
    
    if using_sqlite then

        search_for_user_insensitive:bind{domain_id = domain_id, name = string.lower(name)}
        local row = search_for_user_insensitive:first_row()
        if row then return true else return false end
    
    else
        return domains[domain_id].users[string.lower(name)] ~= nil
    end
end
