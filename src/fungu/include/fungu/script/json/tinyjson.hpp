 ...
 *
 * 09 Feb 2009 - added AnyTraits template parameter (Graham Daws
 * 07 Feb 2009 - using std::iterator_traits (Graham Daws)
...
namespace json
{
	...
	template< typename Char, typename AnyTraits >
	class grammar : public BOOST_SPIRIT_CLASSIC_NS::grammar< grammar< Char, AnyTraits > >
	{
	public:

		// ---[ TYPEDEFINITIONS ]---------------------------------------------------------------------------------
    
        typedef typename AnyTraits::any_type                        any;
        
		typedef	boost::shared_ptr< any >							variant;			//  pointer to a shared variant
	...
	protected:
		...
		struct push_string
		{
			...
			void operator() (Iterator szStart, Iterator szEnd) const
			{
				...
				std::basic_string< typename std::iterator_traits<Iterator>::value_type > strString;

				while(szStart < szEnd)
				{
					// 2.1: if it's no escape code, just append to the resulting string...

					if(*szStart != static_cast< typename std::iterator_traits<Iterator>::value_type >('\\'))
					{
					...
					else
					{
						...
						switch(*szStart)
						{
							...
							case 'b':
								...
								strString.push_back(static_cast< typename std::iterator_traits<Iterator>::value_type >('\b'));
								break;
							
							case 'f':
								
								strString.push_back(static_cast< typename std::iterator_traits<Iterator>::value_type >('\f'));
								break;
							
							case 'n':
								
								strString.push_back(static_cast< typename std::iterator_traits<Iterator>::value_type >('\n'));
								break;
							
							case 'r':
								
								strString.push_back(static_cast< typename std::iterator_traits<Iterator>::value_type >('\r'));
								break;
							...
							case 'u':
							{
								...
								int iUnicode;
								std::basic_istringstream< Char >(std::basic_string< typename std::iterator_traits<Iterator>::value_type >(&szStart[1], 4)) >> std::hex >> iUnicode;

								szStart += 4;

								// 2.1.2.2: append the unicode int...

								strString.append(unicodecvt< typename std::iterator_traits<Iterator>::value_type >::convert(iUnicode));
							}
						}
					}
				...
				m_stack.push(variant(new any(strString)));
			}
		};
		...
		struct push_double
		{
			...
			{
				m_stack.push(variant(new any(dValue)));
			}
		};
		...
		struct push_int
		{
			...
			{
				m_stack.push(variant(new any(iValue)));
			}
		};
		...
		struct push_boolean
		{
			...
			void operator() (Iterator szStart, Iterator /* szEnd */ ) const
			{
				...
				m_stack.push(variant(new any(*szStart == static_cast< typename std::iterator_traits<Iterator>::value_type >('t'))));
			}
		};
		...
		struct create_pair
		{
			...
			void operator() (Iterator /* szStart */, Iterator /* szEnd */ ) const
			{
				...
				std::basic_string< typename std::iterator_traits<Iterator>::value_type > strName;

				try
				{
					strName = AnyTraits::template cast< std::basic_string< typename std::iterator_traits<Iterator>::value_type > >(*m_stack.top());
				}
				catch(typename AnyTraits::exception_type &) { /* NOTHING */ }

				m_stack.pop();

				// 3: push a pair of both on the stack...

				m_stack.push(variant(new any(pair(strName, var))));
			}
		};
		...
		struct begin_array
		{
			...
			{
				m_stack.push(variant(new any(array_delimiter())));
			}
		};
		...
		struct end_array
		{
			...
			void operator() (Iterator /* cCharacter */) const
			{
				...
				variant varArray(new any(array()));
				...
				while(!m_stack.empty())
				{
					...
					if(AnyTraits::template cast< array_delimiter >(var.get()) != NULL)
					{
						break;
					}
					...
					AnyTraits::template cast< array >(varArray.get())->push_front(var);
				}
		...
		struct begin_object
		{
			...
			void operator() (Iterator /* cCharacter */) const
			{
				m_stack.push(variant(new any(object_delimiter())));
			}
		};
		...
		struct end_object
		{
			...
			void operator() (Iterator /* cCharacter */) const
			{
				...
				variant varObject(new any(object()));
				...
				while(!m_stack.empty())
				{
					...
					if(AnyTraits::template cast< object_delimiter >(var.get()) != NULL)
					{
						break;
					}
					...
					pair * pPair = AnyTraits::template cast< pair >(var.get());
					if(!pPair)
					{
						/* BIG PROBLEM!! */
					...
					AnyTraits::template cast< object >(varObject.get())->insert(std::make_pair(pPair->first, pPair->second));
				}
	...
	public:
		...
		class definition
		{
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_object;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_array;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_pair;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_value;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_string;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_number;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_boolean;
			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > m_null;

		public:

			BOOST_SPIRIT_CLASSIC_NS::rule< SCANNER > const & start() const { return m_object; }
	...
	template <typename Iterator, typename AnyTraits>
	typename json::grammar<typename std::iterator_traits<Iterator>::value_type,AnyTraits>::variant parse(Iterator szFirst, Iterator szEnd)
	{
		// 1: parse the input...
        
		typename json::grammar<typename std::iterator_traits<Iterator>::value_type,AnyTraits>::stack st;
		json::grammar<typename std::iterator_traits<Iterator>::value_type,AnyTraits> gr(st);

		BOOST_SPIRIT_CLASSIC_NS::parse_info<Iterator> pi = BOOST_SPIRIT_CLASSIC_NS::parse(szFirst, szEnd, gr, BOOST_SPIRIT_CLASSIC_NS::space_p);

		// 2: skip any spaces at the end of the parsed section...

		while((pi.stop != szEnd) && (*pi.stop == static_cast<typename std::iterator_traits<Iterator>::value_type>(' ')))
		{
			++pi.stop;
		}
		...
		{
			return typename json::grammar<typename std::iterator_traits<Iterator>::value_type,AnyTraits>::variant(new typename AnyTraits::any_type);
		}
