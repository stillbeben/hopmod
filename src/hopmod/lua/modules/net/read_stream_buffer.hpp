...
private:
    ...
    void read_complete(boost::system::error_code ec, 
                       std::size_t bytes_transferred, ReadHandler handler)
    {
        m_consume = bytes_transferred;
        handler(ec, boost::asio::buffer_cast<const PodType *>(
            *m_buffer.data().begin()), static_cast<std::size_t>(bytes_transferred/sizeof(PodType)));
        unlock_read();
    }
