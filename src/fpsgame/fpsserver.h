...
struct fpsserver : igameserver
{
    ...
    fpsserver() : notgotitems(true), notgotbases(false), gamemode(0),
        ...
        demoplayback(NULL), nextplayback(0),
        capturemode(*this), ctfmode(*this), 
        smode(NULL),auth(*this),respawn_locked(false),gametimefreeze(0),
    ...
    void parsepacket(int sender, int chan, bool reliable, ucharbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        #define QUEUE_MSG { if(demorecord || hasnonlocalclients()) while(curmsg<p.length()) ci->messages.add(p.buf[curmsg++]); }
        #define QUEUE_BUF(size, body) { \
            if(demorecord || hasnonlocalclients()) \
            { \
                curmsg = p.length(); \
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            case SV_POS:
            {
                ...
                getuint(p);
                if((demorecord || hasnonlocalclients()) && (ci->state.state==CS_ALIVE || ci->state.state==CS_EDITING))
                {
                    ci->position.setsizenodelete(0);
            ...
            case SV_INITC2S:
            {
                ...
                if((smode && !smode->canchangeteam(ci, ci->team, sent_team)) && m_teammode)
                {
            ...
            case SV_FORCEINTERMISSION:
                if(!hasnonlocalclients()) startintermission();
                break;
            ...
            case SV_SETMASTER:
            {
                ...
                if(allow)
                {
                    if(ci->hidden_priv)
                    {
                        ...
                        bool haspass = masterpass[0] && checkpassword(ci, masterpass, text);
    ...
    static void hashpassword(int cn, int sessionid, const char *pwd, char *result)
    {
