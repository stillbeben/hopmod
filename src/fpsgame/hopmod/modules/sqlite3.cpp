...
private:
    ...
    cubescript::module::symbol_delegate< cubescript::function1<_void,const std::string &> > m_func_open;
    cubescript::module::symbol_delegate< cubescript::function2<_void,const std::string &,const std::string &> > m_func_eval;
    cubescript::module::symbol_delegate< cubescript::function0<_void> > m_func_close;
    cubescript::module::symbol_delegate< cubescript::function1<_void,const std::string &> > m_func_onerror;
    cubescript::module::symbol_delegate< cubescript::function0<sqlite_int64> > m_func_last_rowid;
};
...
_void sqlite3db::eval(const std::string & statement,const std::string & rowcode)
{
    ...
    for(int i=1; i<=paramcount; i++)
    {
        ...
        if(name)
        {
            ...
            {
                const std::string value=get_parent_domain()->require_symbol(&name[1])->value();
                sqlite3_bind_text(sqlstmt,i,value.c_str(),value.length(),NULL);
            }
        }
    }
...
_void new_sqlite3(std::list<std::string> & arglist,cubescript::domain * aDomain)
{
    std::string name=cubescript::functionN::pop_arg<std::string>(arglist);
    aDomain->get_first_nontmp_domain()->register_symbol(name,new sqlite3db);
}

class sqlite3db_module:public cubescript::module
{
public:
    void register_symbols(cubescript::module::domain_accessor * domain)
    {
        static cubescript::module::symbol_delegate< cubescript::functionV<_void> > func_new_sqlite3(&new_sqlite3);
        domain->register_symbol("new_sqlite3",&func_new_sqlite3);
    }
};

IMPLEMENT_CUBESCRIPT_MODULE(sqlite3db_module);
